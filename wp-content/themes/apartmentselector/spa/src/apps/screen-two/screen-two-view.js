...
define(['extm', 'marionette'], function(Extm, Marionette) {
  ...
  UnitViewChildView = (function(_super) {
    ...
    UnitViewChildView.prototype.template = '<!--<div class="box psuedoBox {{classname}} pull-left">{{count}}</div>--> <div class="pull-left light"> <h5 class="rangeName bold">{{range}}</h5> <div class="small">{{rangeNo}}</div> </div> <div class="pull-right box {{classname}}">{{count}}</div> <div class="clearfix"></div> </div>';
